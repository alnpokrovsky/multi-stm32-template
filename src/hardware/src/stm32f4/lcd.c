...
static const colormodel_descript LCD_COLOR_MODELS[] = {
...
void lcd_init(const LCD_Layer * l1, const LCD_Layer * l2) {
	...
	FRAMEBUFFER_1 = malloc(LCD_SIZE * LCD_COLOR_MODELS[l1->m].pixSize);
	FRAMEBUFFER_2 = malloc(LCD_SIZE * LCD_COLOR_MODELS[l2->m].pixSize);
	...
	for (size_t i = 0; i < INIT_SIZE; i++) {
		...
		lcd_command(INIT[i].cmd, INIT[i].delay, arg_count, args);
	}
}

void lcd_setLayer(const LCD_Layer * l) {
	/* The pixel input format */
	LTDC_LxPFCR(l->layerN) = LCD_COLOR_MODELS[l->m].pixFormat;
	...
	uint32_t pitch = l->width * LCD_COLOR_MODELS[l->m].pixSize;
...
#pragma weak lcd_handler = null_handler
